...
export default class EmbedCommand extends SlashCommandBase {
    ...
    public async callback(appInteractions: AppInteractions, channelObject: TextChannel, member: GuildMember, args: Array<any>): Promise<void> {
        ...
        let embeds: Array<MessageEmbed> | undefined = undefined;

        switch (args[0]) {
            case 1:
                embeds = this._modpackEmbed(channelObject);
                break;
            case 2:
                embeds = this._launcherEmbed(channelObject);
                break;
            case 3:
                embeds = this._rulesEmbed(channelObject);
                break;
        }

        if (embeds !== undefined) {

            const channel = await channelObject.client.channels.fetch(args[1]) as TextChannel;

            for (let embed of embeds) {
                channel.send(embed);
            }

            appInteractions.callbackMessage("已成功嵌入式訊息");

        } else {
            appInteractions.callbackMessage("嵌入式訊息失敗");
        }
    }

    private _modpackEmbed(channelObject: TextChannel): Array<MessageEmbed> {
    ...
    private _launcherEmbed(channelObject: TextChannel): Array<MessageEmbed> {
    ...
    private _rulesEmbed(channelObject: TextChannel): Array<MessageEmbed> {

        const gameChannel = channelObject.guild.channels.cache.get("661526275726245921")?.toString();
        const exclusiveCoordinationRoomChannel = channelObject.guild.channels.cache.get("805431146786717727")?.toString();
